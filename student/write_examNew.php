    ...
    <div class="row">
      <?php foreach ($questions as $idx=>$q): ?>
        <div class="col-xl-12 question-card <?= $idx==0?'active':'' ?>" data-index="<?= $idx ?>" data-qid="<?= $q['id'] ?>">
          ...
          <?php shuffle($q['options']); foreach ($q['options'] as $opt): ?>
            <div class="card container fixed-scroll2">
              <div class="card-body profile-card pt-4">
                <input class="form-check-input option-radio" style="font-size: 40px;" type="radio" name="q<?= $q['id'] ?>" value="<?= $opt['id'] ?>" data-is_correct="<?= $opt['is_correct'] ?>">
                &nbsp;&nbsp;&nbsp;<label class="form-check-label"><p style="font-size: 40px;" dir="auto"><strong><?= htmlspecialchars($opt['option_text']) ?></strong></p></label>
              </div>
            </div>
          <?php endforeach; ?>
        </div>
