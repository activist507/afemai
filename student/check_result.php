    ...
    <div class="container">
        ...
        <div class="card mt-3">
            <div class="card-header py-0 d-flex flex-center bg-secondary justify-content-center">
                <h5 class="mb-0 text-white mb-4 mt-2 text-uppercase text-center"><strong> Check Your Result</strong>
                </h5>
            </div>
    ...
    $('#check_result_form').submit(function(e) {
        ...
        if (term == 'none') {
            $.alert({
                ...
                theme: 'bootstrap',
                title: 'Message',
                content: 'Please select a term',
                animation: 'scale',
                type: 'orange'
            })
        } else {
            $.ajax({
                url: 'check_result_db.php',
                type: 'POST',
                dataType: 'JSON',
                cache: false,
                data: {
                ...
                success: function(response) {
                    if (response.status == 'failed') {
                        ...
                        window.location.href = 'result.php?ID=' + response.Student_ID +
                            '&image=' + response.OnlineImage + '&branch=' + branch_ +
