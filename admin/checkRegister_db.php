...
if(isset($_POST['type']) && $_POST['type'] == 'checkRegister'){
  ...
  $branch = $_POST['branch'];
  $stdLists = staff_class_student($class_id,$conn,$branch);
  $title=$sdatee[1];
  ...
  $html .= '</tr></thead>';
  foreach ($stdLists as $std) {
    $id = $std['Student_ID'];
    $name = $std['Fullnames'];
    $html .= "<tr><td nowrap='nowrap'>$name</td>";

    $sql2 = $conn->query(" SELECT w.work_date,CASE WHEN a.status = 1 THEN 1 ELSE 0 END AS status
      FROM workdays w LEFT JOIN attendance_students a ON w.work_date = a.date AND a.studentID = '$id' AND a.class = '$class_id' AND a.branch = '$branch'
