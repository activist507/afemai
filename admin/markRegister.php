<?php 
  $pres = 1; 
  if(isset($_GET['submitClass'])){
	$staff_class = $_GET['_class'];
  } else {
	$staff_class = 'Creche';
  }
  $dateTop = date('Y-m-d');
				...
				<div class="card">
					<div class="card-body">
						...
						<form action="" class="row g-3" method="GET">
							<input type="hidden" name="markRegister">
								...
								<div class="tom-select-custom">
									<select name="_class" id="_class" class="form-control" data-bs-toggle="tooltip"
										...
										<option value="<?= $class->class?>"
											<?php ($staff_class == $class->class) ? print "selected": ""?>>
											<?= $class->class?>
										</option>
										<?php }?>
									</select>
							...
							<div class="col-4 mb-2 mb-sm-0">
								<button class="btn btn-primary" type="submit" value="submitClass"
									name="submitClass">Select
									Class</button>
							</div>
						</form>
